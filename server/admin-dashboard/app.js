...
function renderRoutesList() {
  ...
  routes.forEach((r, ridx) => {
    ...
    const stopNames = r.stopIds.map(id => {
      const stop = stops.find(s => s.id === id);
      return stop ? stop.name : id;
    }).join(' → ');
    
    const textSpan = document.createElement('span');
    textSpan.innerHTML = `<b>${r.name}</b> <small>(${stopNames})</small>`;
    textSpan.style.flex = '1';
    
    const deleteBtn = document.createElement('button');
    deleteBtn.textContent = 'Delete';
    deleteBtn.dataset.idx = ridx;
    deleteBtn.className = 'deleteRoute';
    deleteBtn.style.backgroundColor = '#ff6b6b';
    deleteBtn.style.color = 'white';
    deleteBtn.style.padding = '4px 12px';
    deleteBtn.style.border = 'none';
...
$('placeResults').onchange = () => { focusToSelectedPlace(); };
